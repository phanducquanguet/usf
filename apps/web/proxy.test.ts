...
describe("proxy legacy workspace route redirects", () => {
  ...
  // The fork has no public marketing host: the root of every host serves the
  // customer portal (or redirects logged-in users), so legacy app routes and
  // root redirects behave the same on any host.
  it("redirects root on any host for logged-in users (fork has no marketing host)", () => {
    expect(redirectLocation("/", sessionCookies, "uniai.unicomhub.com")).toBe(
      "https://uniai.unicomhub.com/acme/issues",
    );
  });

  it("redirects explicit legacy app routes regardless of host", () => {
    expect(
      redirectLocation("/issues/ABC-123", sessionCookies, "uniai.unicomhub.com"),
    ).toBe("https://uniai.unicomhub.com/acme/issues/ABC-123");
  });
});
...
describe("proxy root and locale handling", () => {
  ...
  it("forwards a supported cookie locale on login requests", () => {
  ...
  it("falls back to vi for unsupported cookie locales", () => {
