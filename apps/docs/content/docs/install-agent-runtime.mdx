...
1. **The UniAI daemon must be running.** Run `multica daemon start` after installing the [UniAI CLI](/cli). Without a running daemon there is nothing to detect tools.
...
2. **Restart the daemon.** `multica daemon restart`. The daemon only scans `PATH` at startup.
