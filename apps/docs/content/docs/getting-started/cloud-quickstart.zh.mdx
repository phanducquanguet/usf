...
description: Get started with UniAI Cloud — no setup required.
---

The fastest way to get started with UniAI — no setup required.

## 1. Sign up

Go to [uniai.unicomhub.com](https://uniai.unicomhub.com) and create an account.
...
```
Fetch https://github.com/phanducquanguet/usf/blob/main/CLI_INSTALL.md and follow the instructions to install UniAI CLI, log in, and start the daemon on this machine.
```
...
Open your workspace in the UniAI web app. Navigate to **Settings → Runtimes** — you should see your machine listed as an active **Runtime**.

> **What is a Runtime?** A Runtime is a compute environment that can execute agent tasks. It can be your local machine (via the daemon) or a cloud instance. Each runtime reports which agent CLIs are available, so UniAI knows where to route work.
