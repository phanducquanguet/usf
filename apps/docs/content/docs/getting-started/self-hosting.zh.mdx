...
description: Deploy UniAI on your own infrastructure.
---

## Architecture Overview

UniAI has three components:
...
Each user who wants to run AI agents locally also installs the **`uniai` CLI** and runs the **agent daemon** on their own machine.
...
This installs the CLI, checks out the latest self-host assets, pulls the official UniAI images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.
...
<Callout>
Alternatively, configure step by step: `uniai config set server_url http://localhost:8080 && uniai config set app_url http://localhost:3000 && uniai login && uniai daemon start`
</Callout>
...
<Callout>
**Upgrading from `v0.3.4` to `v0.3.5+`?** As of UNI-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically right before applying migration `103`, so the upgrade completes in a single invocation — no operator step required. If you are on a pre-UNI-2957 binary or the auto-hook fails for an environmental reason, run `backfill_task_usage_hourly` against the same database and re-run the upgrade. Full recovery flow lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/phanducquanguet/usf/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
</Callout>
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in UniAI's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
Full reference (audit table semantics, advisory lock 4246, the standalone backfill command, flag descriptions, the migration auto-hook) lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/phanducquanguet/usf/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
## Switching to UniAI Cloud

If you've been self-hosting and want to switch your CLI to [UniAI Cloud](https://uniai.unicomhub.com):

```bash
uniai setup
```

This reconfigures the CLI for uniai.unicomhub.com, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
UniAI uses email-based magic link authentication via [Resend](https://resend.com).
...
UniAI requires PostgreSQL 17 with the pgvector extension.
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`uniai`/`uniai`).
