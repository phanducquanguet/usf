---
title: Architecture
description: Technical architecture of the UniAI platform.
---

## Overview

UniAI is a Go backend + monorepo frontend (pnpm workspaces + Turborepo) with shared packages.
...
- **Entry points** (`cmd/`): `server` (HTTP API), `uniai` (CLI + daemon), `migrate`
