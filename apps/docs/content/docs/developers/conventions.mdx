...
If you write UniAI code, change a translation, or write Chinese product copy, this is the page to reference.
...
### The core distinction: everyday noun vs UniAI-specific term

UniAI's product nouns split into two categories:
...
- **UniAI-specific term** — a concept no local word carries (`skill`), or a schema-level identifier a user may have to type or match (`todo`, `in_progress`, `task_id`). Render as **lowercase English** so it reads like a type name.
...
- **Code references and literal commands** stay English: `uniai issue ...`, the `/issue` Slack and Lark slash command.
- **`skill`** keeps lowercase English in Chinese text — a UniAI-specific concept with no established Chinese term; titles may capitalize as `Skills`.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. The everyday task word is what people already say for this. `skill` is a UniAI-specific concept where no local word carries the meaning.
...
| Category | Terms |
| --- | --- |
| Brands | **UniAI**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
