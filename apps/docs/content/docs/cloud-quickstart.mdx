...
This page walks you end-to-end through UniAI Cloud — **sign up → install the CLI → start the [daemon](/daemon-runtimes) → create an [agent](/agents) → assign your first [task](/tasks)**. Takes about 5 minutes.
...
Sign up at [uniai.unicomhub.com](https://uniai.unicomhub.com). You can log in with email (6-digit verification code) or Google.
...
1. Configure the CLI to connect to UniAI Cloud
...
4. When done, it reports the result back to UniAI (status becomes `completed` or `failed`, depending on whether auto-retry kicks in)
