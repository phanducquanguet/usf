---
title: GitHub 集成
description: 一次性连接 GitHub App，之后 PR 的分支名、标题或正文里写了 issue 编号（例如 UNI-123），就会自动挂到那个 issue 上——PR 合并时 issue 自动转 Done。
---

import { Callout } from "fumadocs-ui/components/callout";

在 **Settings → GitHub** 里一次性连一个 GitHub 账号或组织。之后任何 PR 只要分支名、标题或正文里出现 issue 编号（例如 `UNI-123`），就会**自动关联**到那个 [issue](/issues)，出现在 issue 详情页右侧的 **Pull requests** 区块里——PR 合并时，issue 自动转 **Done**。
...
- 大小写不敏感——`uni-123`、`UNI-123`、`Uni-123` 都能匹配
- 有边界——左侧 `\b`、右侧只接数字，避免误抓 `v1.2-3`、email 地址等
- 限定到本工作区——只匹配本工作区的 [issue prefix](/workspaces)。前缀是 `UNI` 的工作区里，PR 出现 `FOO-1` 不会匹配，即使数字撞另一个 issue 也不会
- 自动去重——`Closes UNI-1, UNI-1` 只关联一次

一个 PR 里**可以同时引用多个 issue**。比如 `Closes UNI-1, UNI-2`：PR 同时关联两个 issue，合并时两个 issue 都会转 `Done`。
...
PR **关闭但没合并**——只更新 PR 卡片的状态为 `Closed`，issue 状态不变。"关闭但不合并"语义因团队而异，UniAI 不替用户做决定。
...
- **Commit message 里的编号**——只扫 PR 的分支 / 标题 / 正文。一个 commit message 写 `UNI-123: fix login` 不会触发关联，除非同样的字符串也出现在 PR 标题或正文里
- **PR 评论里的编号**——只扫 PR 自己的元数据，后续的 GitHub comment 不读
- **App 没安装的仓库里的 PR**——没 App，UniAI 收不到 webhook
- **手动把 PR 关联到 issue**——暂时没有这个 UI。如果你们的约定把编号放到 UniAI 不扫的地方，请改放到 PR 标题或正文里

## 断开连接

**Settings → GitHub** 里没有 installation 列表——现有 installation 直接到 GitHub 上管理：

- **从 GitHub 卸载** —— 个人在 `https://github.com/settings/installations`、组织在 `https://github.com/organizations/<org>/settings/installations` 卸载 UniAI App。UniAI 收到 `installation.deleted` webhook 后立刻删行；任何已打开的 Settings tab 实时更新，不用刷新
- **UniAI 这边的断开是 admin only** —— GitHub tab 上的 Disconnect 控件对非 admin 不显示；主开关关掉时 Disconnect 仍然可用，方便 admin 一键关闭功能后再单独清理已连接的 installation
...
- GitHub App 申请的是 PR 和 Metadata 的 **只读** 权限。UniAI 从不向 GitHub 推 commit、评论或 status check

## Self-Host 配置

如果你在 UniAI Cloud 上，集成已经配好——跳过本节。
...
| 字段 | 值 |
|---|---|
| **GitHub App name** | 任何能辨识的名字，例如 `UniAI` 或 `UniAI (staging)` |
| **Homepage URL** | 你的 UniAI 前端，例如 `https://multica.example.com` |
...
**Webhook secret ≠ Client secret。** App 设置页里两个字段紧挨着。**Webhook secret** 用于签 `pull_request` payload，这才是 UniAI 需要的那个；**Client secret** 是 OAuth 用的，和本集成无关。混淆这两个会得到「每条 webhook 都 `401 invalid signature`」的诡异症状。
</Callout>
...
- `/api/webhooks/github` 直接返回 **`503 github webhooks not configured`**——UniAI 在 secret 没配置时拒绝处理事件，不会出现「没 secret 也接受 webhook」的安全坑

`GITHUB_APP_ID` 和 `GITHUB_APP_PRIVATE_KEY` **可选**。配上之后，setup 回调可以用 App JWT 鉴权调用 GitHub `/app/installations/{id}`，安装完成那一刻就拿到真实的组织名/用户名。不配的话，连接卡片会先显示 `已连接到 unknown`，等 GitHub 的 `installation.created` webhook 到达（通常几秒内），UniAI 会刷新 row 并通过 realtime 推送让 Settings 页面无需手动刷新即可更新。私钥从 App 设置页 **Private keys → Generate a private key** 生成，把整段 PEM（含 `-----BEGIN/END RSA PRIVATE KEY-----` 两行）粘到 env 里，保留换行。
...
- **不读 CI / check 状态**——只镜像 PR 本身，构建状态、reviewer 评论、reviewer 列表都没接进 UniAI
- **没有工作区级别的 merge → status 映射配置**——默认固定是 `merged → done`（cancelled 除外）。可配置映射是后续迭代
- **同 issue 多 PR 时，merge 行为偏激进**——两个 PR 都引用 `UNI-123` 时，第一个 merge 就把 issue 转 Done。"等所有关联 PR 都解决再推进 issue 状态"的优化已经在做了

## 下一步

- [Issues](/issues) —— PR 引用的 issue 编号（`UNI-123`）的来源
