...
这个形状是有意的 —— 跟 UniAI 里 agent provider 的设计完全一致：一个 `type` 判别字段 + 一个有类型的 payload。schema 保持稳定，未来加"Notion 页面"、"Google Doc"、"上传文件"、"外部 URL"都是小型的、增量的改动。
...
跟 `github_repo` 相比的取舍是有意的：只有绑定的那台守护进程会使用这个本地目录执行任务，而且 **同一个目录上的任务串行执行**，不再并行。换来的是：你的现有 checkout、当前分支、未提交的脏改动一切照旧 —— UniAI 不会重新 clone。
...
浏览器没法读 OS 路径，所以网页端没有文件夹选择器。请在拥有该路径的守护进程所在的机器上，用 **CLI** 添加目录：
...
`--daemon-id` 可以通过 `uniai daemon list` 拿到。CLI 也接受 `--ref '<json>'` 这种通用形式，直接传 payload。
...
黑名单故意做得激进 —— 选你自己的 home 目录就意味着 UniAI 的运行时文件会出现在你账号的根下，这不会是任何人想要的结果。请选一个子目录（一般就是你具体的项目 checkout）。

### 每台守护进程每个项目最多一条

同一项目上 **每台守护进程最多只能绑一个 `local_directory`**。在同一台机器上想再加一条会被 API 用 `409` 拒绝。
...
4. UniAI 自己的运行时产物（`output/`、`logs/`、`.gc_meta.json`）放在 **你目录之外** 的一个独立 envRoot 里。
...
- **不会自动开 PR。** 任务结束后，改动就停在它实际做出来的分支上 —— UniAI 不会 push、不会开 PR。需要 push、需要 PR 的话请自己来。
...
在网页端打开 *新建项目* 时，Status / Priority / Lead 旁边有一个 **Repos** 标签。选已绑到工作区的仓库（或者粘贴一个临时 URL），项目创建的瞬间它们就以 `github_repo` 资源的形式挂上去。
...
守护进程在 checkout 这一侧也是配套的：当任务带着项目级别的 `github_repo` URL 进来时，这些 URL 会被合并进每工作区的白名单，并在智能体启动前同步进本地仓库缓存。所以即便某个项目仓库 URL 没被绑到工作区，`uniai repo checkout` 也能正常处理它，不会以"未配置"为由拒绝。白名单的划分只是内部实现：工作区绑定的 URL 和任务级别的 URL 分开记录，工作区仓库列表的刷新不会顺手吊销某条项目 URL。

## 当前**不**做的事

- **跨项目共享**。每条资源现在只能挂在一个项目上。
- **按 skill 划定资源可见性**。所有资源对智能体本次运行里的每个 skill 都可见；按类型过滤是后续工作。
- **缓存 / 同步**。`github_repo` 目前只是元数据 —— checkout 仍然按需走 `uniai repo checkout`。Notion / Google Docs 之类的文档文本缓存会随对应类型一起到位。
