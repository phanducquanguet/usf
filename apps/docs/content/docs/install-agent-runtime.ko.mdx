...
1. **UniAI 데몬이 실행 중이어야 합니다.** [UniAI CLI](/cli)를 설치한 후 `uniai daemon start`를 실행하세요. 데몬이 실행 중이지 않으면 도구를 감지할 주체가 없습니다.
...
2. **데몬을 재시작하세요.** `uniai daemon restart`를 실행하세요. 데몬은 시작 시에만 `PATH`를 스캔합니다.
