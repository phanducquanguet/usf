...
**Người vận hành tự host lưu ý**: giữ `MULTICA_DEV_VERIFICATION_CODE` trống trên triển khai công khai. Nếu bật mã kiểm thử cố định, bất kỳ ai xin được mã đều đăng nhập được bằng giá trị đó khi `APP_ENV` không phải production. Xem Thiết lập xác thực tự vận hành.
</Callout>

### Google OAuth

Bấm **Đăng nhập với Google** và đi qua callback OAuth chuẩn. Tự vận hành cần cấu hình `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` và redirect URI — xem Thiết lập xác thực tự vận hành.
...
- Thiết lập xác thực tự vận hành — cấu hình email, OAuth và danh sách đăng ký khi tự vận hành
