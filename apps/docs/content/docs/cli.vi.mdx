...
<Callout type="info">
Các lệnh `list` (`uniai issue list`, `autopilot list`, `project list`, v.v.) mặc định in ID ngắn sẵn sàng chép-dán — mã issue kiểu `MUL-123` cho issue, tiền tố UUID ngắn cho phần còn lại. Đối số `<id>` ở các lệnh tiếp theo nhận cả ID ngắn lẫn UUID đầy đủ, nên luồng điển hình là `uniai issue list` → chép mã → `uniai issue get MUL-123`. Truyền `--full-id` cho lệnh list khi cần UUID chính tắc.
</Callout>
