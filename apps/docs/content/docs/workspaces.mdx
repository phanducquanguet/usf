...
- **Issue prefix** — the prefix for every issue number in the workspace (the `UNI` in `UNI-123`). Uppercase letters and digits, up to 10 characters.

<Callout type="warning">
**Avoid changing the issue prefix.** Issue numbers are rendered with the current prefix — change it and `UNI-5` instantly becomes `NEW-5`. Every external link, Slack mention, and historical reference in comments breaks against the old number. Treat the issue prefix as "set at creation, never touched."
</Callout>
...
Every issue created in a workspace is automatically assigned a number in the format `<prefix>-<digits>` — `UNI-1`, `UNI-2`, `UNI-3`. A few properties:
...
- **Never reclaimed on delete** — delete `UNI-5` and the next new issue is `UNI-6`, not `UNI-5`.
