...
description: Common issues when self-hosting UniAI — symptoms, causes, how to diagnose, how to fix.
---

import { Callout } from "fumadocs-ui/components/callout";

Look up issues by symptom. Each entry gives you **symptom / likely causes / how to diagnose / how to fix**. If your situation isn't listed, open an issue on [GitHub](https://github.com/phanducquanguet/usf/issues).

## Daemon can't connect to the server

**Symptom**: [`uniai daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).
...
3. **Token expired or invalid** — you never ran `uniai login`, or the PAT was revoked
...
```bash
uniai daemon logs --lines 100    # look for daemon-side errors
...
uniai workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `MULTICA_SERVER_URL` and restarting the daemon** (`uniai daemon restart`) and **signing in again** (`uniai logout && uniai login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `uniai daemon status` shows the daemon `online`.
...
```bash
uniai daemon status --output json       # runtime list + last_seen_at
uniai agent list                         # check agent archived state
uniai issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `uniai agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `uniai agent restore <id>`
- Runtime not registered → `uniai daemon restart`, and the daemon will re-register
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | The relay's receive connector doesn't allow your subnet to relay to external recipients (most common for anonymous internal relays talking to outside domains) | Either restrict invites to internal recipients, or add the UniAI host's subnet to the Exchange "Anonymous Users → Relay" permission list |
...
- `Unable to relay` → either restrict to internal recipients or grant the UniAI host's IP relay permission on the Exchange receive connector
...
Since UNI-2957 the migrate command runs an idempotent monthly-slice backfill (under advisory lock 4246) automatically immediately before applying migration `103`, so v0.3.4 → v0.3.5+ direct upgrades complete in a single `migrate up` invocation. If you are still seeing this error you are either on a pre-UNI-2957 binary or the hook itself failed — check the migrate logs for an earlier `task_usage hourly rollup hook` line.

**How to fix**:

1. If you are on a pre-UNI-2957 binary and cannot upgrade the binary first, run the standalone backfill against the same database (idempotent, safe to interrupt, safe to re-run):
...
**Symptom**: `uniai server` or `uniai daemon start` fails with `address already in use`.
...
|---|---|---|
| **Daemon** | `~/.multica/daemon.log` (background mode) or foreground stdout | `uniai daemon logs -f --lines 100` |
...
For more detailed daemon logs, move it from background to foreground: `uniai daemon stop && uniai daemon start --foreground`. Running in the foreground streams logs live to your terminal instead of the rotated file.
