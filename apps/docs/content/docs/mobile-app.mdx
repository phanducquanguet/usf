...
description: How to build the open-source UniAI iOS app on your own iPhone — no App Store yet.
---

import { Callout } from "fumadocs-ui/components/callout";

UniAI's iOS client is open-source and lives in the [main repo](https://github.com/phanducquanguet/usf) alongside web, desktop, and backend. It isn't on the App Store yet — until that changes, anyone who wants it on their iPhone builds from source. The build takes about 10–20 minutes the first time and ~2 minutes after that, and it talks to the same backend as [uniai.unicomhub.com](https://uniai.unicomhub.com) so your existing account just works.

<Callout type="info">
This page is for **personal use**. App developers should read [`apps/mobile/README.md`](https://github.com/phanducquanguet/usf/blob/main/apps/mobile/README.md) in the repo — it covers the dev / staging variants and the full script matrix.
</Callout>
...
- The UniAI source code checked out:
...
There is no auto-update yet. When the UniAI codebase moves forward, pull and rebuild:
...
The iOS app is still moving fast — the team prefers ship-and-iterate over App Store review cycles right now. A TestFlight beta is the most likely next step before a full App Store release. Until then, the self-build path above is the only way to use UniAI on iOS.

If you'd like to be notified when TestFlight opens, watch the [GitHub repo](https://github.com/phanducquanguet/usf).
