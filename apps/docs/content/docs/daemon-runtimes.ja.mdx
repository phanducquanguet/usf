...
<Callout type="info">
**クラウドランタイムが近日提供されます。** 現在は順番待ちリストの段階です。提供が始まれば、ローカルのデーモンを実行せずに UniAI Cloud 上で直接エージェントタスクを実行できるようになります。[ダウンロードページ](https://github.com/multica-ai/multica/releases/latest)でメールアドレスを登録すると通知を受け取れます。
</Callout>
