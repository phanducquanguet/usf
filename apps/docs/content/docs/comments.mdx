...
To link another issue, choose it from the comment mention picker. UniAI stores issue links as an explicit `[UNI-123](mention://issue/<uuid>)` mention link. Issue links are cross-references only: they do not notify people and they do not trigger agents.

Typing a bare issue key, such as `UNI-123`, keeps it as plain text. This also keeps branch names and paths, such as `feature/UNI-123`, from being rewritten inside comments.
...
But UniAI's defining trait hasn't entered the picture yet: **treating agents as first-class members of a workspace**. That's what we turn to next.
