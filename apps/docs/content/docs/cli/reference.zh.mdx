---
title: CLI Reference
description: Complete command reference for the UniAI CLI and agent daemon.
---
...
When connecting to a self-hosted UniAI instance, point the CLI to your server before logging in:
...
uniai config set server_url wss://api.example.com/ws
```
...
```bash
uniai workspace watch <workspace-id>
uniai workspace unwatch <workspace-id>
```
...
```bash
uniai workspace member invite alice@example.com
uniai workspace member invite alice@example.com <workspace-id> --role admin
uniai workspace member invite alice@example.com --output json
```
...
```bash
uniai workspace update <workspace-id> --name "Acme Eng"
uniai workspace update <workspace-id> \
...
```bash
cat <<'CTX' | uniai workspace update <workspace-id> --context-stdin
...
表格输出默认显示可直接复制到后续命令的 issue `KEY`（例如 `UNI-123`）；需要完整 UUID 时使用 `--full-id`。Available filters: `--status`, `--priority`, `--assignee` / `--assignee-id`, `--project`, `--limit`. 在重名 workspace 下用 `--assignee-id <uuid>` 可以精确锁定一个成员或 agent。

### Get Issue

```bash
uniai issue get UNI-123
uniai issue get <uuid>
uniai issue get <id> --output json
```

`<id>` 同时接受 issue key（`uniai issue list` 表格里直接显示，例如 `UNI-123`）和完整 UUID（给 `list` 加 `--full-id` 可显示）。同样的规则适用于下面 `update` / `assign` / `status` / `comment` / `subscriber` / `runs` 等接受 `<id>` 的命令。
...
Flags: `--title` (required), `--description`, `--status`, `--priority`, `--assignee` / `--assignee-id`, `--parent`, `--project`, `--due-date`. 脚本里如果已经拿到了 UUID（例如来自 `uniai workspace member list --output json`），传 `--assignee-id <uuid>`（与 `--assignee` 互斥）以精确锁定。
