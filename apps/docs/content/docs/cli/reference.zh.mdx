...
```bash
uniai issue get MUL-123
uniai issue get <uuid>
uniai issue get <id> --output json
```

`<id>` 同时接受 issue key（`uniai issue list` 表格里直接显示，例如 `MUL-123`）和完整 UUID（给 `list` 加 `--full-id` 可显示）。同样的规则适用于下面 `update` / `assign` / `status` / `comment` / `subscriber` / `runs` 等接受 `<id>` 的命令。
