...
非 Backlog 状态的 issue 一旦分配给小队，UniAI 会立刻给**队长智能体**入队一个 `task`（不是给每个成员都入一个）。整个流程是这样的：

1. **队长领走 task。** 队长所在的 daemon 在下次轮询时把 task 领走，和普通智能体的分配流程一样。
2. **队长拿到 briefing。** 领走的瞬间，UniAI 会在队长的系统提示后面追加三段内容——详见下文 [队长每次执行看到的内容](#队长每次执行看到的内容)。
3. **队长发一条"派活"评论。** 评论里用 roster 里给好的 mention markdown `@` 选中的成员——这个 `@` 会触发被派的成员入队新 `task`。
4. **队长记录 evaluation：** `uniai squad activity <issue-id> action --reason "..."`。这一行会写进 issue 的 activity 时间线，方便人类回溯队长确实评估过这一次触发。
...
- **Squad Instructions（小队自定义指令）** —— 你为这个小队写的私货（在详情页里编辑，或用 `uniai squad update --instructions`）。用来写路由规则（"DB 相关派给 Alice，前端派给 Bob"）、上报策略，或者任何 issue 本身不会有的背景。
...
| 队长自己发的评论 | **不会**——硬编码防自触发 |
| 评论里只有 issue 互链 `[UNI-123](mention://issue/...)` | **会**——issue 引用不算路由 |
...
**归档小队**（`uniai squad delete <id>`，或详情页的 Archive 按钮）：
...
| `uniai squad get <id>` | 查看小队的名字、队长、描述、instructions |
| `uniai squad create --name "..." --leader <agent>` | 创建小队（owner / admin）|
| `uniai squad update <id> [--name X] [--description X] [--instructions X] [--leader Y] [--avatar-url Z]` | 修改一个或多个字段 |
| `uniai squad delete <id>` | 归档（软删除）——同时把当前分配给小队的 issue 转给队长 |
| `uniai squad member list <id>` | 列出小队成员 |
| `uniai squad member add <id> --member-id <uuid> --type agent\|member [--role "..."]` | 加成员（owner / admin）|
| `uniai squad member set-role <id> --member-id <uuid> --member-type agent\|member --role "..."` | 不移除成员，直接修改 role |
| `uniai squad member remove <id> --member-id <uuid> --type agent\|member` | 移除成员（**不能移除队长**——先换队长）|
| `uniai squad activity <issue-id> <action\|no_action\|failed> --reason "..."` | 队长每次结束前由它自己调用 |

`--leader` 接受智能体名字或 UUID；其它 ID 从 `uniai agent list --output json`、`uniai workspace member list --output json`、`uniai squad list --output json` 拿。
