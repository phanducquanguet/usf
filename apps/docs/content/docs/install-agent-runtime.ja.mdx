...
1. **UniAI デーモンが実行中である必要があります。** [UniAI CLI](/cli) をインストールした後に `uniai daemon start` を実行してください。デーモンが実行されていなければ、ツールを検出する主体がありません。
...
2. **デーモンを再起動してください。** `uniai daemon restart` を実行してください。デーモンは起動時にのみ `PATH` をスキャンします。
