...
1. **UniAI デーモンが実行中である必要があります。** [UniAI CLI](/cli) をインストールした後に `multica daemon start` を実行するか、デーモンを自動的に起動する [UniAI デスクトップアプリ](/desktop-app)を使用してください。デーモンが実行されていなければ、ツールを検出する主体がありません。
