...
the `--version` output of the `uniai` binary it would re-exec. When they differ
— `uniai update`, a re-download, a local `make build` — it waits for any
...
check, and `uniai daemon status` shows why it's still on the old version.
...
```bash
MULTICA_DAEMON_AUTO_RELOAD=0 uniai daemon start
# or
uniai daemon start --no-auto-reload
# or persist it
uniai config set disable_auto_reload true
```
...
**without restarting**, so subsequent tasks pick up the new CLI while UniAI's
...
Table output shows a routable issue `KEY` such as `UNI-123`; copy that key into follow-up commands like `issue get`, `issue comment list`, `issue status`, or `--parent`. Add `--full-id` when you need canonical UUIDs. Available filters: `--status`, `--priority`, `--assignee` / `--assignee-id`, `--project`, `--metadata`, `--limit`. Use `--assignee-id <uuid>` for unambiguous filtering when names overlap.
...
uniai project update <id> --due-date 2026-04-15
```
...
```bash
LANG=zh_CN.UTF-8 uniai issue get UNI-9999   # 错误信息显示为中文
```
...
MULTICA_DEBUG=1 uniai issue update UNI-1234 --title "x"
```
