...
describe("WelcomeAfterOnboarding", () => {
  ...
  describe("runtime path", () => {
    it("creates a Helper agent then opens a blocking modal with starter cards", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/welcome to UniAI/i)).toBeInTheDocument();
      });

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.runtime_id).toBe("rt-1");
      expect(agentArgs.name).toBe("UniAI Helper");
      expect(agentArgs.instructions).toContain("UniAI Helper");

      // 3 starter card titles come from HELPER_STARTER_PROMPTS (TS const,
      // EN under the test's en locale).
      expect(
        screen.getByText("Introduce UniAI to me"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Walk me through the core features"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Show me what UniAI can do for me — as slides"),
      ).toBeInTheDocument();
    });

    it("reuses an existing UniAI Helper agent instead of creating duplicates", async () => {
      mockListAgents.mockResolvedValueOnce([
        {
          id: "agent-existing",
          name: "UniAI Helper",
    ...
    it("selecting cards then clicking Assign creates one issue per pick and navigates to the first", async () => {
      ...
      // Toggle two cards.
      fireEvent.click(screen.getByText("Introduce UniAI to me"));
      fireEvent.click(
        screen.getByText("Show me what UniAI can do for me — as slides"),
      );
      ...
      expect(titles).toEqual([
        "Introduce UniAI to me",
        "Show me what UniAI can do for me — as slides",
      ]);
  ...
  describe("skip path", () => {
    it("provisions install-runtime → agent-guide → follow-up comment, then opens the celebration Modal", async () => {
      ...
      expect(secondCall![0].title).toBe(
        "Step 2 — Create your first UniAI Agent",
      );
