...
function renderInput(props: Partial<React.ComponentProps<typeof ChatInput>> = {}) {
  ...
  const view = render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={onSend} uploadEnabled agentName="UniAI" {...props} />
    </I18nProvider>,
  );
  return { onSend, ...view };
}
...
function element(props: Partial<React.ComponentProps<typeof ChatInput>>) {
  return (
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={vi.fn()} uploadEnabled agentName="UniAI" {...props} />
    </I18nProvider>
  );
}
...
describe("ChatInput focusRequest", () => {
  it("focuses the editor when focusRequest becomes a non-zero value (new chat)", () => {
    const { rerender } = render(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="UniAI" focusRequest={0} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="UniAI" focusRequest={1} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="UniAI" focusRequest={2} />
      </I18nProvider>,
    );
