...
print_remote_server_token_hint() {
  ...
  printf "     1. On your local computer, open ${CYAN}https://uniai.unicomhub.com/settings?tab=tokens${RESET}\n"
  printf "        and create a token under ${BOLD}Settings > API Tokens${RESET}.\n"
  printf "     2. On this server, run:\n"
  printf "        ${CYAN}uniai login --token <YOUR_TOKEN>${RESET}\n"
  printf "        ${CYAN}uniai daemon start${RESET}\n"
  printf "\n"
}
