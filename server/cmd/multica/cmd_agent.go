...
func init() {
	...
	// `uniai agent env set <id>` — that path admits the agent owner or a
...
const (
	defaultCloudServerURL = "https://uniai.unicomhub.com"
	defaultCloudAppURL    = "https://uniai.unicomhub.com"
)
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --thinking-level, --service-tier, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `uniai agent env set <id>`)")
	}
