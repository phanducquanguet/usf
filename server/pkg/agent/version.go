...
// the uniai CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `uniai issue create` failure — see PR
...
// MinHandoffCLIVersion is the lowest uniai CLI version whose daemon renders
...
var (
	ErrCLIVersionMissing = errors.New("uniai CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("uniai CLI version is below required minimum")
)
