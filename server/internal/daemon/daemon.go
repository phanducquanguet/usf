...
// `uniai repo checkout`. Under Codex's workspace-write sandbox a linked
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by UniAI Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// runUpdate downloads targetVersion from GitHub Releases and returns the
// human-readable output. The caller is responsible for the
...
func (d *Daemon) runUpdate(targetVersion string) (string, error) {
	// This fork does not publish a Homebrew tap; brew-located binaries are
	// replaced in place by the direct download like any other install.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// `uniai` (pre-rename), while the installed file carries whichever name
	// it was installed under — self-updated installs keep the old `uniai`
...
// (`uniai`). Agent prompts and skills reference the CLI by name, and the
// installed file may carry either one (self-updated installs keep the old
// `uniai` file name), so both spellings must resolve on the agent's PATH.
