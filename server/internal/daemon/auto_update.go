...
// ParseSelfVersion pulls the version out of `uniai --version` output, whose
// first line is rendered by cmd/multica's version template:
//
//	uniai 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)
...
func ParseSelfVersion(raw string) string {
	...
	// Accept both the current `uniai` binary name and the pre-rename
	// `multica` one: self-updated installs keep the old file/report name.
	if fields := strings.Fields(line); len(fields) >= 2 && (fields[0] == "uniai" || fields[0] == "multica") {
		return fields[1]
	}
	return line
}
...
// different uniai binary". It runs two independent checks on one goroutine,
...
func (d *Daemon) autoUpdateLoop(ctx context.Context) {
	...
	if reloadEnabled {
		d.logger.Info("auto-reload: watching the uniai binary on disk",
...
func (d *Daemon) trySelfReload(ctx context.Context) {
	...
	reason := fmt.Sprintf("uniai binary on disk reports %s, running %s", onDisk, d.cfg.CLIVersion)
