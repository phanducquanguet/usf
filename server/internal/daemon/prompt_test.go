...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (uniai issue create has no --subscriber flag)
		"CC exception",
		...
		// fixed prefix.
		"uniai issue create --output json",
		"JSON response",
		"identifier",
...
func TestBuildChatPromptChannelAwareness(t *testing.T) {
	t.Run("slack-backed prompt teaches both read commands", func(t *testing.T) {
		...
		for _, want := range []string{"Slack", "NOT in UniAI", "uniai chat history", "uniai chat thread", "Do NOT narrate"} {
			if !strings.Contains(out, want) {
	...
	t.Run("top-level mention starts with history", func(t *testing.T) {
		...
		if !strings.Contains(out, "top level: start with `uniai chat history`") {
	...
	t.Run("in-thread mention starts with thread", func(t *testing.T) {
		...
		if !strings.Contains(out, "inside a thread: start with `uniai chat thread`") {
	...
	t.Run("web-only session has no channel block", func(t *testing.T) {
		...
		if strings.Contains(out, "uniai chat history") {
...
//     Any IM reply leaves UniAI, where the upload has nothing to bind to.
...
func TestBuildChatPromptTwoLayerChannelPolicy(t *testing.T) {
	...
	// `uniai attachment upload` on purpose — to state that it does not apply
	...
	const uploadGuidance = "run `uniai attachment upload <local-path>`"
	const historyGuidance = "uniai chat history"
...
func TestBuildChatPromptFeishuIgnoresChatInThread(t *testing.T) {
	...
	for _, unwanted := range []string{"uniai chat thread", "uniai chat history"} {
...
func TestBuildPromptDefaultScansRootsFirst(t *testing.T) {
	...
	for _, s := range []string{
		"uniai issue comment list issue-default-1 --roots-only --summary --compact --output json",
		"--since",
	} {
		if !strings.Contains(out, s) {
	...
	if strings.Contains(out, "uniai issue comment list issue-default-1 --output json") {
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "uniai issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --compact --output json") {
	...
	if strings.Contains(out, "uniai issue comment list "+issueID+" --since "+since+" --output json") {
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "uniai issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
	...
	if strings.Contains(out, "uniai issue comment list "+issueID+" --roots-only --summary --output json") {
...
func TestBuildCommentPromptCoalescedIDsOnlyFallback(t *testing.T) {
	...
	t.Run("with since anchor", func(t *testing.T) {
		...
		want := "uniai issue comment list issue-fallback-1 --since 2026-08-03T06:00:00Z --compact --output json"
		if !strings.Contains(out, want) {
...
func assertBoundedIDOnlyFallback(t *testing.T, out string) {
	...
	for _, want := range []string{
		"uniai issue comment list issue-fallback-1 --thread <comment-id> --tail 30 --compact --output json",
		"accepts a reply id",
		"Next reply cursor",
		"--before-id",
...
func TestBuildCommentPromptCrossThreadFansOutReplies(t *testing.T) {
	...
	if strings.Contains(out, "uniai issue comment add") {
