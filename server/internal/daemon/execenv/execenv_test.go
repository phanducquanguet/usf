...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"uniai autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `uniai issue get") {
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"uniai issue create --title",
		"uniai issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"uniai repo checkout <url>",
		"uniai issue status <id> <status>",
		"uniai issue comment add <issue-id>",
	...
	for _, banned := range []string{
		"uniai issue list [--status",
		"uniai issue label list",
		"uniai issue subscriber list",
		"uniai label list",
		"uniai workspace member list",
		"uniai agent list",
		"uniai squad list",
		"uniai issue runs",
		"uniai issue run-messages",
		"uniai attachment download",
		"uniai autopilot list",
		"uniai autopilot create",
		"uniai autopilot update",
		"uniai autopilot trigger",
		"uniai autopilot delete",
		"uniai project get",
		"uniai project resource list",
		"uniai issue assign",
		"uniai issue label add",
		"uniai issue label remove",
		"uniai issue subscriber add",
		"uniai issue subscriber remove",
		"uniai issue comment delete",
		"uniai label create",
	} {
		if strings.Contains(s, banned) {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "UniAI Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "uniai issue get") {
		t.Error("should reference uniai CLI even without skills")
	}
...
// user-installed skill already sitting at the natural slug pushes UniAI's
...
func TestWriteContextFilesFrontmatterNameFollowsCollisionSlug(t *testing.T) {
	...
	// UniAI's copy landed at the fallback slug and names itself after it.
...
func TestInjectRuntimeConfigQoderCN(t *testing.T) {
	...
	if !strings.Contains(string(content), "UniAI Agent Runtime") {
		t.Error("AGENTS.md missing meta skill header")
	}
}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `uniai issue comment add`
			...
			mustContain := []string{
				...
				"post it with `uniai issue comment add` using",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `uniai issue comment add`",
...
// heredoc on `uniai issue create`, the flags were silently swallowed into
...
// workspace-write and rejects mutation commands (e.g. `uniai issue create`)
...
// inside a UniAI task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigBriefOmitsResumedThreadAnchor(t *testing.T) {
	...
	for _, want := range []string{
		...
		"uniai issue comment list " + issueID + " --thread thread-root-1 --tail 30 --compact --output json",
	} {
...
func TestInjectRuntimeConfigAssignmentTriggerScansRootsFirst(t *testing.T) {
	...
	for _, banned := range []string{
		"uniai issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigCatchUpScansRootsFirst(t *testing.T) {
	...
	if strings.Contains(s, "uniai issue comment list issue-1 --recent") {
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"uniai issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"uniai issue metadata delete <issue-id> --key <k>",
	}
	...
	withSection := wantSection{
		present: []string{
			...
			"uniai issue metadata delete",
			"the `multica-working-on-issues` skill",
		},
	}
	withoutSection := wantSection{
		// We can't simply require `uniai issue metadata list` absent
	...
	cases := []struct {
	...
	}{
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"Read the metadata bag (`uniai issue metadata list`)",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if !strings.Contains(s, "Read the metadata bag (`uniai issue metadata list`)") {
