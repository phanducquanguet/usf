...
func writeHeader(b *strings.Builder) {
	...
	b.WriteString("You are a coding agent in the UniAI platform. Use the `uniai` CLI to interact with the platform.\n\n")
}
...
func BuildTaskInitiatorBlock(initiatorType, initiatorName, initiatorEmail string) string {
	...
	b.WriteString("The initiator — not the runtime owner — is who you are answering: apply any per-person privacy or access rules your instructions define. Your UniAI credentials stay scoped to the runtime owner, and initiator attribution does not change what you may read or write; do not assume the initiator can see everything you can.\n\n")
	return b.String()
}
...
// preserved: each `uniai issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `uniai issue comment add
// --help`.
//
...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `uniai --help` or `uniai <command> --help`.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `uniai issue get <id> --output json` — full issue.\n")
	b.WriteString("- `uniai issue comment list <issue-id> [--roots-only] [--summary] [--thread <comment-id> [--tail N] | --recent N] [--since <RFC3339>] --output json` — thread-aware comment reads. Bound a wide read with `--roots-only --summary` (roots plus `reply_count` / `last_activity_at`, clipped bodies); bound a deep one with `--thread <id> --tail N`; add `--compact` to any JSON read to drop echoed/null/bookkeeping fields. Careful with `--recent N`: it caps THREADS, not comments, and can return the whole history on a small issue. Resolved-thread folding, paging cursors, and full flag semantics: `--help`.\n")
	...
	b.WriteString("- `uniai issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>]` — update fields; pass `--parent \"\"` to clear parent.\n")
	b.WriteString("- `uniai issue status <id> <status>` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
	b.WriteString("- `uniai issue children <id> [--output json]` — list a parent's sub-issues grouped by stage.\n")
	b.WriteString("- `uniai issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — post a comment. Agent-authored bodies MUST use `--content-file`; see `## Comment Formatting` for why. `uniai issue comment add --help` for full flags.\n")
	b.WriteString("- `uniai issue metadata list <issue-id> [--output json]` — list KV metadata.\n")
	b.WriteString("- `uniai issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `uniai issue metadata delete <issue-id> --key <k>` — remove a key.\n")
	b.WriteString("- `uniai repo checkout <url> [--ref <branch-or-sha>]` — repository checkout on a dedicated branch.\n\n")
	...
	if ctx.IsSquadLeader {
		...
		b.WriteString("- `uniai squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
	}
}
...
// every CLI other than `uniai issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `uniai --help` or `uniai <command> --help`.\n\n")
	b.WriteString("### Core\n")
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `uniai repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		...
		b.WriteString("For `github_repo` resources, use `uniai repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Only what a future run will actually re-read — short values, never secrets or long content. Overwrite or `uniai issue metadata delete` stale keys. Full write discipline: the `multica-working-on-issues` skill.\n\n")
}
...
//   - Slack: the conversation lives in the channel and `uniai chat history` /
//     `uniai chat thread` can fetch it — see buildChatPrompt, which hands the
...
//     in the provider session, and UniAI ships no history reader for Feishu
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `uniai` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `uniai issue list --output json` or `uniai issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `uniai workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `uniai repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	...
	b.WriteString("- Run exactly one `uniai issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `uniai issue get`, `uniai issue status`, or `uniai issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `uniai issue create` succeeded.\n")
...
const AutopilotIssueCommandsGuard = "Do not run `uniai issue get`, `uniai issue comment add`, or `uniai issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue"
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("1. Read the issue (`uniai issue get`) to understand the context.\n")
	b.WriteString("2. Read the metadata bag (`uniai issue metadata list`) — best-effort, empty `{}` and CLI failures are normal. What to look for: `## Issue Metadata`.\n")
	...
	if ctx.IsSquadLeader {
		b.WriteString("5. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `uniai squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `uniai issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n")
	} else {
		b.WriteString("5. **Post your final results as a comment — this step is mandatory**: post it with `uniai issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). `## Output` states why this call is the only delivery channel.\n")
	}
	b.WriteString("6. Before exiting, pin or clear a metadata key via `uniai issue metadata set`/`delete` only if it clears the bar in `## Issue Metadata`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write.\n\n")
	...
	b.WriteString("- Before step 4, run `uniai issue status <issue-id> in_progress`.\n")
	if ctx.IsSquadLeader {
		...
		b.WriteString("- When done, run `uniai issue status <issue-id> in_review`.\n")
	}
	b.WriteString("- If blocked, run `uniai issue status <issue-id> blocked`, and post a comment explaining the blocker unless your Agent Identity forbids issue comments.\n\n")
...
//
// The index itself stays because it is the one skill listing UniAI controls.
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("Fetch issue/comment attachments via the authenticated CLI (`uniai attachment --help`); never open UniAI resource URLs directly.\n")
...
// writeAlwaysUseCLI emits the "must go through the uniai CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `uniai` CLI\n\n")
	b.WriteString("Access UniAI platform resources only through the `uniai` CLI — never `curl` / `wget`. For anything the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		...
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `uniai issue create`, using the created issue's `identifier` from JSON output (fall back to its `id`; never assume a workspace issue prefix such as `MUL-`).\n")
		...
		b.WriteString("**Delivering files here:** your stdout is text-only. A file that belongs to the new issue goes on the `uniai issue create` call itself via `--attachment <path>`; never put its path in the description or in your stdout line.\n")
	case kindChat:
		...
		// non-empty channel type means the reply leaves UniAI for an external
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** this %s conversation is text-only — UniAI cannot push a file you produced back into it. `uniai attachment upload` does NOT apply: it binds to a UniAI chat reply, which this is not. Say in words what you produced and where it can be obtained; never upload and then write as though the file arrived, and never link its local path.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `uniai attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `uniai issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `uniai squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `uniai issue comment add`.** The user does NOT see your terminal output or run logs — only comments on the issue.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `uniai issue comment add` (repeatable) — the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
