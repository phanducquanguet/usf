...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"uniai issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`uniai issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `uniai issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`uniai issue status <this-issue-id> in_review`") {
...
func TestColdCommentsHintPointsAtTriggeringThread(t *testing.T) {
	...
	if !strings.Contains(hint, "uniai issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
...
func TestIssueWorkflowHonorsAgentIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Before step 4, run `uniai issue status <issue-id> in_progress`.",
		...
		"When done, run `uniai issue status <issue-id> in_review`.",
		"If blocked, run `uniai issue status <issue-id> blocked`, and post a comment explaining the blocker unless your Agent Identity forbids issue comments.",
	} {
		if !strings.Contains(out, want) {
	...
	for _, banned := range []string{
		"4. Run `uniai issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `uniai issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestSquadLeaderIssueWorkflowKeepsParentInProgress(t *testing.T) {
	...
	if strings.Contains(out, "When done, run `uniai issue status <issue-id> in_review`") {
...
func TestChatOutputDoesNotRequireIssueComment(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"do not call `uniai issue comment add`",
...
func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	...
	const brief = "# UniAI Agent Runtime\n\nbrief body line"
...
func TestWriteRuntimeConfigFileReplacesExistingBlock(t *testing.T) {
	...
	const newBrief = "## New UniAI brief\n\nfresh body"
...
func TestWriteRuntimeConfigFileIgnoresStrayEndMarkerBeforeBegin(t *testing.T) {
	...
	// real block authored by an earlier UniAI run below.
	const userDoc = "# Repo CLAUDE.md\n\nExample of what UniAI writes:\n" +
		runtimeMarkerEnd + "\n\n# Real config below\n"
	original := userDoc +
		runtimeMarkerBegin + "\nFIRST BRIEF\n" + runtimeMarkerEnd + "\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
...
// `codex` run started by the user after a UniAI task must see the same
// file the user wrote.
...
func TestMultiThreadReplyInstructionsFanOut(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"uniai issue comment add",              // embedded example commands
...
func TestBriefSkillsListIsNamesOnly(t *testing.T) {
	...
	ctx := TaskContextForEnv{
		...
		AgentSkills: []SkillContextForEnv{
			{
				Name:        "PR Review",
				Description: "Use when reviewing a pull request for the UniAI project.",
				Content:     "---\nname: pr-review\n---\n\nbody",
			},
		},
	}
