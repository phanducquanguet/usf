...
func TestBriefSurfaceDeliveryPolicy(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"comment": {
			mustHave: []string{"`--attachment <path>` to `uniai issue comment add`"},
			mustNot:  []string{"uniai attachment upload"},
		},
		"assignment": {
			mustHave: []string{"`--attachment <path>` to `uniai issue comment add`"},
			mustNot:  []string{"uniai attachment upload"},
		...
		"chat_direct": {
			mustHave: []string{"`uniai attachment upload <local-path>`"},
		...
		"chat_slack": {
			...
			mustNot:  []string{"run `uniai attachment upload"},
		},
		"chat_feishu": {
		...
		"quickcreate": {
			mustHave: []string{"your stdout is text-only", "`uniai issue create` call itself via `--attachment <path>`"},
			mustNot:  []string{"uniai attachment upload"},
		},
	}
