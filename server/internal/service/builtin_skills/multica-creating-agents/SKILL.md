...
uniai agent env get <agent-id> --output json  # plaintext env (owner/admin only, agents denied)
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`uniai agent env get`). It is gated to workspace **owner/admin**
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`uniai agent env set`), which is owner/admin-only
