...
description: "Use when creating, inspecting, or debugging a UniAI agent definition via the `uniai agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(uniai *)
---

# Creating UniAI agents

This is the contract for UniAI's agent-creation path: what the create entry
...
```bash
uniai agent get <agent-id> --output json      # full persisted agent record
uniai agent skills list <agent-id> --output json   # current skill bindings
uniai agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`uniai agent create`). At task claim time the daemon
...
```bash
uniai agent create --name <name> --runtime-id <runtime-id> \
...
`uniai agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
uniai agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
uniai agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
uniai agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
uniai agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`uniai agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`uniai agent env set`), which carries the same
...
```bash
uniai agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
uniai agent update <agent-id> --mcp-config-stdin --output json
uniai agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
```bash
uniai agent skills add <agent-id> --skill-ids <skill-id> --output json
uniai agent skills list <agent-id> --output json
```
...
- `uniai agent create` — inserts a new agent row.
- `uniai agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `uniai agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `uniai agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
