...
description: "Use when a UniAI runtime or daemon misbehaves: agent not running, task not claimed, runtime offline, workdir or session reuse, repository checkout."
...
The daemon injects a task-scoped `mat_` credential for UniAI API commands and a private task-local UniAI configuration root. Inside that managed task context:

- API commands such as `issue list`, `issue get`, and `issue runs` use the injected task identity and never fall back to the daemon Owner's saved UniAI profile.
- `config show` and `config set` operate only on task-local UniAI state. A missing task config root fails closed.
