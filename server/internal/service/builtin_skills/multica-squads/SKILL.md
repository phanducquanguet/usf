---
name: multica-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a UniAI squad, including how leader routing picks who runs."
...
uniai issue comment list <issue-id> --roots-only --summary --output json
uniai issue comment list <issue-id> --thread <thread-id> --tail 30 --output json
```
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `uniai issue status` no.
