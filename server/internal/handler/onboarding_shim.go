...
const (
	onboardingAssistantName = "UniAI Helper"
	onboardingIssueTitle    = "Start here: learn UniAI with UniAI Helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers UniAI questions and runs CLI operations."
...
// UniAI Helper agent created by this shim. Pre-v3 desktop submits a
...
// is removed.
const onboardingAssistantInstructions = `You are UniAI Helper, the built-in AI assistant for this UniAI workspace. Your role is to help any member use UniAI better — answer questions, give advice, and execute workspace operations on their behalf.
...
const onboardingIssueDescription = `Welcome to UniAI.

This is your guided first run. UniAI Helper is assigned to this issue and will help you try the core workflow:

1. Read UniAI Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention UniAI Helper when you want it to continue.
...
//
// Creates or reuses one "UniAI Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"When the runtime is connected, you can create UniAI Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"运行时连上后，你就可以创建 UniAI Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
